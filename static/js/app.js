...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!checkAuth()) {
        return; // Если не авторизован - редирект, не загружаем данные
    }
    
    // Только если авторизован - инициализируем приложение
    initNavigation();
    initModals();
    loadDashboard();
    loadSettings();
    ...
    try {
        ...
        todayLessons.forEach(lesson => {
            ...
            row.innerHTML = `
                ...
                <td>
                    ${lesson.student_name}
                    ${lesson.is_moved && !lesson.completed ? '<span style="color: #f59e0b; font-size: 0.75rem; margin-left: 0.5rem;">Перенесенный</span>' : ''}
                </td>
...
async function loadSettings() {
    try {
        ...
        // Отображаем ссылку для учеников
        if (data.invite_link) {
